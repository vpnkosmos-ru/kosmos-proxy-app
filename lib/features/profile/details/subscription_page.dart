...
class _SubscriptionPageState extends ConsumerState<SubscriptionPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final expiresAt = SubscriptionExpirationInfo.fromProfile(profile)?.expiresAt ?? subscription?.expire;
    final devices = _deviceLimit(profile.populatedHeaders);
    final expired = subscriptionAccessState(profile) == SubscriptionAccessState.expired;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Gap(18),
          if (expired) ...[
            const Text('Оплатите доступ', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
            const Gap(6),
            const Text('Доступ закончился. Продлите подписку, чтобы снова подключиться.'),
            const Gap(10),
            FilledButton(
              onPressed: () => openExternalKosmosLink(context, kosmosCabinetUri),
