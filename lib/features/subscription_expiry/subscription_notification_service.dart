...
class SubscriptionNotificationService {
  ...
  Future<void> refreshFromProfile(ProfileEntity? profile) async {
    ...
    // A successful profile refresh atomically replaces the old server
    // metadata. In particular, a stale positive expiry must never survive a
    // response that no longer contains valid subscription information.
    if (info != null) {
      await _prefs.setString(_expiryKey, jsonEncode(info.toJson()));
    } else {
